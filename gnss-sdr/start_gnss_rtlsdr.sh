...
################################################################################
# Complete GNSS-SDR System for RTL-SDR - ONE SCRIPT
#
# This script starts EVERYTHING you need for RTL-SDR GPS tracking:
# 1. WebSocket bridge on port 8766 (for web UI)
# 2. Continuous recording and processing loop using RTL-SDR
#
# Usage: ./start_gnss_rtlsdr.sh
# Stop: Press Ctrl+C
################################################################################
...
echo "========================================================================"
echo "Complete GNSS-SDR System Startup (RTL-SDR)"
echo "========================================================================"
echo ""
...
pkill -9 -f "python.*rtlsdr" 2>/dev/null || true
pkill -9 -f "gnss-sdr" 2>/dev/null || true
...
# Check RTL-SDR
echo "🔍 Checking RTL-SDR connection..."
if ! python3 -c "import SoapySDR; devices = SoapySDR.Device.enumerate('driver=rtlsdr'); exit(0 if devices else 1)" 2>/dev/null; then
    echo "❌ ERROR: RTL-SDR not found!"
    echo "   Please check:"
    echo "   • RTL-SDR is connected via USB"
    echo "   • librtlsdr is installed"
    echo "   • Try: brew install librtlsdr"
    exit 1
fi
echo "✓ RTL-SDR found"
echo ""
...
while true; do
    ...
    # Step 1: Record with RTL-SDR
    echo "📡 Recording 5 minutes (longer for PVT fix) of GPS samples with RTL-SDR..."
    ...
    python3 record_iq_samples_rtlsdr.py /tmp/gps_iq_samples.dat 300 &
    RECORD_PID=$!
