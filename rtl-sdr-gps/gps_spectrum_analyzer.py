#!/usr/bin/env python3
"""
RTL-SDR GPS Spectrum Analyzer for Jamming Detection
Analyzes RTL-SDR IQ recordings (8-bit uint8) to detect GPS jamming signatures
...
RTL-SDR Format:
- Sample format: 8-bit unsigned integers (uint8)
- Two bytes per sample: I (byte 0) and Q (byte 1)
- Value range: 0-255 (centered at 127.5)
- Conversion: uint8 → float complex (normalize to -1.0 to +1.0)

Usage:
    python3 gps_spectrum_analyzer.py <recording.dat> [options]

Author: Adapted from SDRplay GPS analyzer for RTL-SDR hardware
"""
...
class GPSSpectrumAnalyzer:
    ...
    def load_samples(self, filename, max_samples=None, skip_seconds=0.3):
        """Load RTL-SDR 8-bit IQ samples from file and convert to complex float

        RTL-SDR format:
        - Interleaved IQ: [I0, Q0, I1, Q1, I2, Q2, ...]
        - 8-bit unsigned integers (uint8)
        - Range: 0-255 (centered at 127.5)
        - Conversion: (value - 127.5) / 128.0 → range [-1.0, +1.0]
        """
        print(f"Loading RTL-SDR samples from: {filename}")

        # Calculate samples to skip (default: skip first 300ms for SDR settling)
        skip_samples = int(skip_seconds * self.sample_rate)
        skip_bytes = skip_samples * 2  # 2 bytes per IQ sample

        # Calculate how many bytes to read
        read_bytes = -1  # -1 means read all
        if max_samples is not None:
            read_bytes = (max_samples + skip_samples) * 2

        # Read as uint8 array
        raw_data = np.fromfile(filename, dtype=np.uint8, count=read_bytes)

        print(f"  Raw bytes read: {len(raw_data):,}")

        # Skip initial bytes
        if len(raw_data) > skip_bytes:
            raw_data = raw_data[skip_bytes:]
            print(f"  Skipped first {skip_seconds * 1000:.0f} ms ({skip_bytes:,} bytes)")
        ...
        # Method: Separate I and Q, convert to float, normalize
        ...
        # Center at 127.5, divide by 128.0
        I = (I - 127.5) / 128.0
        Q = (Q - 127.5) / 128.0
        ...
        print(f"  Format: RTL-SDR 8-bit IQ (uint8 → complex float)")
